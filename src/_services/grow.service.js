...
function getTimelineEvents(_id) {
    const requestOptions = {method: 'GET', headers: authHeader()};
    return fetch(`http://204.48.25.187/grows/` + _id + `/timeline`, requestOptions).then(handleResponse);
}
...
function updateTimelineEvent(_id, event) {
    ...
    return fetch(`http://204.48.25.187/grows/` + _id + `/timeline/` + event._id, requestOptions).then(handleResponse);
}
...
function deleteTimelineEvent(growId, eventId) {
    ...
    return fetch(`http://204.48.25.187/grows/` + growId + `/timeline/` + eventId, requestOptions).then(handleResponse);
}